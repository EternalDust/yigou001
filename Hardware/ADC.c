...
void ADC_AutoTrig(void)
{
    ...
    while (read_index != write_index) 
    {
        ...
        uint16_t ppval = cal_ppvalue(ring_buffer, BUFFER_SIZE);//峰峰值
        uint16_t ave = average_array(ring_buffer, BUFFER_SIZE);//均值
...
void ADC_SingleTrig(void)
{
    ...
    {
        uint16_t ppval = cal_ppvalue(adcbuff, 50);//峰峰值
        uint16_t ave = average_array(adcbuff, 50);//均值
